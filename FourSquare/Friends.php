...
$Conn6= @mysql_connect('localhost','root','13579') or die('Problem In Server '.mysql_error());
$Dbase6 =@mysql_select_db('foursquare',$Conn6) or die('Problem In Table Inside The Data Base '.mysql_error());


	$Write6= "select * from Friends where (TID = '$FUI' and FID = '$ID') or (TID = '$ID' and FID = '$FUI')";
	$Result6 =@mysql_query($Write6,$Conn6);
		if(mysql_num_rows($Result6) > 0){}
		
		
	else{
	$Write8= "insert into Friends values ('','$FUI','$FUN','$ID','$UName','$FUP','now()') ";
	$Result8 =@mysql_query($Write8,$Conn6);	
	
	
	$Write9= "select * from accounts where ID = '$ID'";	
	$Result9 =@mysql_query($Write9,$Conn6);
		
	while ($Fre5 = mysql_fetch_array($Result9)) {
	$Fre2= 	$Fre5['Friends'];
	$Fre2 = 1 + $Fre2;	
	$Write10= "update accounts set Friends = '$Fre2' where ID = '$ID'";	
	$Result10 =@mysql_query($Write10,$Conn6);
	}
	$Write9= "select * from accounts where ID = '$FUI'";	
	$Result9 =@mysql_query($Write9,$Conn6);

	while ($Fre5 = mysql_fetch_array($Result9)) {
	$Fre2= 	$Fre5['Friends'];
	$Fre2++;	
	$Write10= "update accounts set Friends = '$Fre2' where ID = '$FUI' ";	
	$Result10 =@mysql_query($Write10,$Conn6);
	}
}}
...
<form method="get" action="FindF.php" class="flp" >
			<div><input type="text"  name="Search"   class="Bgm9" id="Find" autocomplete="off" required=""/><label for="Find"> Find Friends By User Name Or Email </label></div>
...
$(this).delay(d).animate({top : tm}, 50, 'easeOutBack');})})
...
$Write4= "select * from Friends where TID = $ID or FID = $ID";
$Result4 =@mysql_query($Write4,$Conn4) or die ('Problem In The Values That Need to Write In Table '.mysql_error());
	...
	else {echo "<div class='Bgm8'><p>Sorry There's No Friends </p></div>";}
