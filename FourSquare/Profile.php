...
$Write2= "select * from uaccounts where ID = '$ID1' or ID = '$ID2' or (Password = '$Password1' and UName = '$UName1')";
$Result2 =@mysql_query($Write2,$Conn2) or die ('Problem In The Values That Need to Write In Table '.mysql_error());
if(mysql_num_rows($Result2) > 0){
$Write3= "select * from accounts where ID = '$ID1' or ID = '$ID2' or (UName = '$UName1' and Password = '$Password1')";
...
$MoviesW = $User[21];
if($_POST['Rem'] == '1'){
if (!isset($_COOKIE['Log'])) {setcookie("Log", "$ID", time()+1000*1000,  "/", null, 0);}
}
	
if ($_POST['Rem'] != '1' ){session_start();$_SESSION['ID'] = $ID;}
...
.Bgm3{
	...
	width: 350px;
	height: 320px;
	box-shadow: 0 10px 50px 0px rgba(0, 0, 0, 0.75);
}
.Bgm4{
	width: 350px;
	background: lightgray;
...
for ($i=1; $i < 4 ; $i++) { 
	echo "
<fieldset class='Bgm'>
	<div class='Bgm1'><a href='' class='Imgh'><img src='FourSquare/Img/New1/$i' class='Bgm3'/></a></div>
	
		<div class='Bgm4'>
			<div class='Bgm5'><a href=''><img src='FourSquare/Img/profile/PIC/1.jpg' class='Bgm7'/></a></div>
			<div class='Bgm6'><p class='P'> Waow Thats Was Imazing Film And Great </p></div>
		</div>
	
</fieldset>";
}
} else {
	echo "<div class='Bgm8'><p> There's No Comments By You In Movies </p></div>";
}
